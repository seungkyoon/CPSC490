...
Y[Y==0] <- -1
ret <- train_model(X, Y, seed = 1, lr = 0.001, epochs = 5000, batch_size = 10,
                   retrain = TRUE)
model <- ret$model

# Save model parameters for model repair
init_weights <- ret$init_weights
init_file <- sprintf("cnn_%d_%d_init.RData", num_filters, kernel_size[1])
save(init_weights, file = init_file)
weights <- ret$weights
weights_file <- sprintf("cnn_%d_%d_weights.RData", num_filters, kernel_size[1])
